...
# Same three methods as demo/allen_cahn.py, same shape, same caveat (they
# train; JCP2024 quotes 54-184 GPU-minutes per run).  Three notes:
...
#    judgement by declaration; the adapter itself builds fine, so the line
#    below does run.
...
compare(pde, mc).table().plot(Path(__file__).with_suffix(".png"),
                              "Merton HJB $d=1$: coding-tree Monte Carlo")
