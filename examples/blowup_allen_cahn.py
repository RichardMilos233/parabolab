"""Allen-Cahn blow-up and integrability window study (JCP2024 Fig. 2 spirit).

Pointwise MC estimates of u(0, 0) as horizon T sweeps from 0.1 to 2.0.
"""
...
if __name__ == "__main__":
    ...
    for d in args.dims:
        ...
        for label, rate in (("jcp", "jcp"), ("rate=1", 1.0)):
            print(
                f"=== Allen-Cahn d={d}, u(0,0) vs T, rho rate {label}, N={args.samples}, seeds={args.seeds} ===",
                flush=True,
            )
            ...
            print(
                f"  -> integrability edge (persistent drift or stderr > 5% of |exact|): T ~ {edges[label]}",
                flush=True,
            )
        ...
        if auth is not None:
            ...
            ax1.plot(
                ...
                alpha=0.6,
                color="tab:green",
                label="authors' deep BSDE (NaN past ~0.65)" if d == 1 else "authors' deep BSDE",
            )
        ...
        ax1.set_title(
            f"Allen-Cahn d={d}: pointwise MC blow-up study (JCP2024 Fig. 2 spirit), N={args.samples:,}"
        )
        ax1.legend(fontsize=8)
        ax1.grid(alpha=0.3)

        for label, sw in sweeps.items():
            ax2.plot(sw.T, sw.max_abs, "-", color=colors[label], label=f"max |H|, {label}")
            ax2.plot(
                sw.T,
                sw.stderr * math.sqrt(args.samples),
                "--",
                color=colors[label],
                alpha=0.6,
        ...
        if auth is not None:
            ...
            for i, T in enumerate(sw.T):
                ...
                if abs(aT[j] - T) < 1e-9:
                    print(
                        f"    T={T:.2f}  ours(jcp) |err|={sw.abs_err[i]:.2e}  authors |err|={abr[j]:.2e}"
                    )
            print(flush=True)
